...
### CRUD Operations
```go
// Option A: use the dynamic Records API (no codegen needed)
...
// Option B: use generated models + typed services (recommended)
posts := models.NewPostsService(client) // generated by pbc-gen
...
// Delete
err = posts.Delete(ctx, created.ID)
```
...
### Batch Operations
```go
createReq, _ := client.Records.NewCreateRequest("posts", map[string]any{"title": "Batch Post"})
updateReq, _ := client.Records.NewUpdateRequest("posts", "ID", map[string]any{"title": "Updated"})
...
See the [full list of error codes](./docs/errors.md) in the source code.

## 📖 Examples

You can find comprehensive, runnable examples in the [`examples/`](./examples/) directory. Each example demonstrates a specific feature or pattern.

| Example | Description |
|---------|-------------|
| **[`basic_crud`](./examples/basic_crud/)** | Essential Create, Read, Update, Delete operations. Start here! |
| **[`auth`](./examples/auth/)** | Authentication patterns (Admin, User, Custom strategies) |
| **[`list_options`](./examples/list_options/)** | Filtering, Sorting, Pagination, and Expanding relations |
| **[`file_management`](./examples/file_management/)** | Uploading and downloading files with records |
| **[`realtime_subscriptions`](./examples/realtime_subscriptions/)** | Subscribing to real-time events (SSE) |
| **[`realtime_chat`](./examples/realtime_chat/)** | A full console-based chat application example |
| **[`batch`](./examples/batch/)** | Executing atomic batch operations (Transactions) |
| **[`streaming_api`](./examples/streaming_api/)** | Handling large responses via streaming (Memory efficient) |
| **[`error_handling`](./examples/error_handling/)** | robust error handling patterns |
...
**Breaking Changes:**
- Pagination helpers removed → Use `ListOptions{Page, PerPage}` and loop if needed
- Typed generic services added → Recommended with codegen models
...
// Typed API (with pbc-gen models)
posts := models.NewPostsService(client)
typed, err := posts.GetList(ctx, opts)
_ = typed
```
